...
public sealed class SmppClient : IDisposable
{
  ...
  //--
  private static TraceSwitch _vTraceSwitch = new("SmppClientSwitch", "SmppClient trace switch");
  ...
  private void OpenSession(SessionBindInfo bindInfo, bool useSeparateConnections, int timeOut)
  {
    ...
    switch (connStateConfig)
    {
      ...
      case (false, _, _):
        try
        {
          var session = SmppClientSession.Bind(bindInfo, timeOut, _vSmppEncodingService);
