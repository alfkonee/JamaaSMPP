...
public class StreamParser : RunningComponent
{
  ...
  private PDU WaitPDU()
  {
    ...
    try
    {
      headerBytes = ReadHeaderBytes();
    }
    catch (TcpIpException tcpIp_ex_1)
    {
      _Log.ErrorFormat("200010:TCP/IP Exception encountered while reading pdu header bytes: {0}", tcpIp_ex_1,
        tcpIp_ex_1.Message);
      if (vTraceSwitch.TraceInfo)
        Trace.WriteLine(string.Format(
          "200010:TCP/IP Exception encountered while reading pdu header bytes:{0};",
          tcpIp_ex_1.Message));
      HandleException(tcpIp_ex_1);
      throw;
    }
    ...
    try
    {
      pdu = PDU.CreatePDU(header, vSmppEncodingService);
    }
    catch (InvalidPDUCommandException inv_ex)
    {
      var iBuffer = new ByteBuffer((int)header.CommandLength);
      iBuffer.Append(header.GetBytes(vSmppEncodingService));
      if (header.CommandLength > 16)
        try
        {
          iBuffer.Append(ReadBodyBytes((int)header.CommandLength - 16));
        }
        catch (TcpIpException tcpIp_ex_3)
        {
          HandleException(tcpIp_ex_3);
        }
    ...
    try
    {
      bodyBytes = ReadBodyBytes((int)header.CommandLength - 16);
    }
    catch (TcpIpException tpcIp_ex_2)
    {
      _Log.ErrorFormat("200012:TCP/IP Exception encountered while reading pdu body bytes: {0}", tpcIp_ex_2,
        tpcIp_ex_2.Message);
      if (vTraceSwitch.TraceInfo)
        Trace.WriteLine(string.Format(
          "200012:TCP/IP Exception encountered while reading pdu body bytes:{0};",
          tpcIp_ex_2.Message));
      HandleException(tpcIp_ex_2);
      throw;
    }
    ...
    {
      var pBuffer = new ByteBuffer((int)header.CommandLength);
  ...
  private byte[] WaitBytes(int length)
  {
    ...
    while (remaining > 0)
    {
      var receiveCount = vTcpIpSession.Receive(bytes, received, remaining);
      if (receiveCount == 0)
  ...
  private void RaisePduErrorEvent(PDUException exception, byte[] byteDump, PDUHeader header, PDU pdu)
  {
    ...
    var e = new PDUErrorEventArgs(exception, byteDump, header, pdu);
  ...
  private void RaiseParserExceptionEvent(Exception exception)
  {
    ...
    var e = new ParserExceptionEventArgs(exception);
