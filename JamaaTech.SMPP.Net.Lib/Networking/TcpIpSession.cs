...
public class TcpIpSession
{
  ...
  public TcpIpSessionProperties Properties => vProperties;

  public SessionType SessionType => vSessionType;
  ...
  private void HandleException(SocketException exception)
  {
     ...
     */
    TerminateSession(SessionCloseReason.ExceptionThrown,
      exception); //Shutdown this session to release claimed resources
  ...
  private void CheckSession()
  {
    ...
    {
      if (!vIsAlive)
        throw new TcpIpSessionClosedException(
          "TCP/IP session cannot send or receive data because it has been shutdown");
    }
  }
  ...
  public void Send(byte[] buffer, int start, int length)
  {
    ...
    try
    {
      vSocket.Send(buffer, start, length, SocketFlags.None);
    }
    catch (SocketException ex)
    {
      HandleException(ex);
  ...
  public int Receive(byte[] buffer, int start, int length)
  {
    ...
    try
    {
      var received = vSocket.Receive(buffer, start, length, SocketFlags.None);
  ...
  {
    var socket = CreateClientSocket(); //Creates a socket to be used as client
    try
    {
      socket.Connect(ipAddress, port);
    } //Connect to the server of the specified ipAddress on the specified port
    catch (SocketException ex)
    {
      HandleConnectionException(ex);
    }

    var session = new TcpIpSession(socket, SessionType.Client);
  ...
  {
    var socket = CreateClientSocket(); //Creates a socket to be used for client connection
    try
    {
