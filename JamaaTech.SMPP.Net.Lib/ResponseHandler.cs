...
public class ResponseHandler
{
  ...
  public void Handle(ResponsePDU pdu)
  {
    ...
    try
    {
      var sequenceNumber = pdu.Header.SequenceNumber;
  ...
  {
    var sequenceNumber = pdu.Header.SequenceNumber;
    var resp = FetchResponse(sequenceNumber);
    if (resp != null) return resp;
    if (timeOut < 5000) timeOut = vDefaultResponseTimeout;
    var waitContext = new PDUWaitContext(sequenceNumber, timeOut);
    vWaitingEvent.WaitOne();
    try
    {
